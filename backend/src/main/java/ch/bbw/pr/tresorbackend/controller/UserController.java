...
public class UserController {
    ...
    public ResponseEntity<String> getUserUuidByEmail(@RequestBody EmailAdress email, BindingResult bindingResult) {
        ...
        System.out.println("UserController.getUserUuidByEmail: input validation passed");

        User user = userService.findByEmail(email.getEmail());
        if (user == null) {
            System.out.println("UserController.getUserUuidByEmail, no user found with email: " + email);
            JsonObject obj = new JsonObject();
            obj.addProperty("message", "No user found with this email");
            String json = new Gson().toJson(obj);

            System.out.println("UserController.getUserUuidByEmail, fails: " + json);
            return ResponseEntity.badRequest().body(json);
        }
        System.out.println("UserController.getUserUuidByEmail, user find by email");
        JsonObject obj = new JsonObject();
        obj.addProperty("answer", user.getUserUuid());
        String json = new Gson().toJson(obj);
        System.out.println("UserController.getUserUuidByEmail " + json);
        return ResponseEntity.accepted().body(json);
    }
